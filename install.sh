...
DISCOVERY_WELL_KNOWN="${PREFIX}/bin/well-known-Z_J_gw_A_lo-discovery"
...
install -m 755 "$ROOT/bin/well-known-Z_J_gw_A_lo-discovery" "$DISCOVERY_WELL_KNOWN"
...
echo "  well-known-Z_J_gw_A_lo-discovery    $("$DISCOVERY_WELL_KNOWN" --version 2>/dev/null | awk '{print $2}')"
echo "  docs/${MAIN_DOC}                    1.0.0"
echo "  docs/zabbix_java_gw_adapter-examples.md  1.0.0"
echo "  template (import manually)          Template App Generic Java Z_J_gw_A_lo 1.0.6"
echo "  well-known Java discovery:  $DISCOVERY_WELL_KNOWN  (--dry-run --report)"
echo "  custom discovery dir:       $PREFIX/lib/zabbix-jmx-discovery/  (copy from examples/)"
